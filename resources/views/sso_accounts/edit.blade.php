        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 text-gray-900 dark:text-gray-100">
                    ...
                    <form action="{{ route('sso_accounts.update', $ssoAccount->id) }}" method="POST" autocomplete="off">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="t-section">
                            ...
                            <div class="t-grid t-grid-2" style="margin-bottom: 10px;">
                                <div>
                                    ...
                                    <input type="text" name="username" id="username" value="{{ old('username', $ssoAccount->username) }}" required class="t-input" autocomplete="new-username">
                                    @error('username') <span class="t-error">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="password" class="t-label">Temporary Password <span style="color: var(--text-muted); font-weight: 400;">(leave blank to keep current)</span></label>
                                    <div style="position: relative; display: flex; align-items: stretch; border: 1px solid var(--border-color); border-radius: 6px; overflow: hidden; background: var(--bg-card); transition: border-color 0.15s, box-shadow 0.15s;" x-data="{ show: false }" onfocusin="this.style.borderColor='#2563eb'; this.style.boxShadow='0 0 0 3px rgba(37, 99, 235, 0.12)';" onfocusout="this.style.borderColor='var(--border-color)'; this.style.boxShadow='none';">
                                        <input :type="show ? 'text' : 'password'" name="password" id="password" value="{{ old('password') }}" class="t-input" style="border: none; box-shadow: none; border-radius: 0;" placeholder="••••••••" autocomplete="new-password">
                                    ...
                                    <label style="display: inline-flex; align-items: center; gap: 6px; cursor: pointer; margin-top: 6px;">
                                        <input type="checkbox" name="password_changed" value="1" {{ old('password_changed', $ssoAccount->password_changed) ? 'checked' : '' }} style="border-radius: 4px; border: 1px solid var(--border-color); width: 14px; height: 14px; cursor: pointer;">
                                        <span style="font-size: 0.8rem; color: var(--text-primary);">Password has been changed by user</span>
                                    </label>
                                </div>
                            </div>
                            <div class="t-grid t-grid-3">
                                <div>
                                    ...
                                    <select name="account_type" id="account_type" required class="t-input">
                                        <option value="New" {{ old('account_type', $ssoAccount->account_type) == 'New' ? 'selected' : '' }}>New</option>
                                        <option value="Transferred" {{ old('account_type', $ssoAccount->account_type) == 'Transferred' ? 'selected' : '' }}>Transferred</option>
                                    </select>
                                ...
                                <div id="transferred_from_container" style="display: none;">
                                    ...
                                    <input type="text" name="transferred_from" id="transferred_from" value="{{ old('transferred_from', $ssoAccount->transferred_from) }}" class="t-input" placeholder="e.g. John Doe">
                                ...
                                <div>
                                    ...
                                    <select name="status" id="status" required class="t-input">
                                        <option value="Active" {{ old('status', $ssoAccount->status) == 'Active' ? 'selected' : '' }}>Active</option>
                                        <option value="Inactive" {{ old('status', $ssoAccount->status) == 'Inactive' ? 'selected' : '' }}>Inactive</option>
                                        <option value="Locked" {{ old('status', $ssoAccount->status) == 'Locked' ? 'selected' : '' }}>Locked</option>
                                    </select>
                        ...
                        <div class="t-section">
                            ...
                            <div class="t-grid t-grid-2" style="margin-bottom: 10px;">
                                    ...
                                    <select name="employee_id" id="employee_id" class="t-input">
                                        ...
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" 
                                                ...
                                                {{ old('employee_id', $ssoAccount->employee_id) == $employee->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="name" id="name" value="{{ old('name', $ssoAccount->name) }}" required class="t-input">
                            ...
                            <div class="t-grid t-grid-3">
                                <div>
                                    ...
                                    <input type="email" name="email" id="email" value="{{ old('email', $ssoAccount->email) }}" class="t-input">
                                ...
                                <div>
                                    ...
                                    <select name="department" id="department" class="t-input">
                                        ...
                                        @foreach(array_keys($hierarchy) as $dept)
                                            <option value="{{ $dept }}" {{ old('department', $ssoAccount->department) == $dept ? 'selected' : '' }}>{{ $dept }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="position" id="position" class="t-input" data-old="{{ old('position', $ssoAccount->position) }}">
                        ...
                        <div class="t-footer">
                            ...
                            <button type="submit" class="t-btn-submit">Update Account</button>
                        </div>
                    </form>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleTransfer() {
                if (typeSelect.value === 'Transferred') {
                    ...
                    if (!'{{ $ssoAccount->transferred_from }}') {
