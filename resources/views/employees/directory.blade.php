<x-app-layout>
    ...
    <div class="py-6" style="background-color: #f8fafc; min-height: calc(100vh - 100px);">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @forelse($employeesByDept as $deptName => $deptEmployees)
                ...
                @endphp
                <div style="margin-bottom: 40px; background: #fff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03); overflow: hidden; border: 1px solid #e2e8f0;">
                    <!-- Department Header -->
                    <div style="background: #1e293b; color: #fff; padding: 15px 25px;">
                                ...
                                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 15px;">
                                    @foreach($posEmployees as $emp)
                                        <div style="background: #f1f5f9; border-radius: 8px; padding: 15px; display: flex; align-items: flex-start; gap: 15px; transition: transform 0.2s, box-shadow 0.2s; border: 1px solid transparent;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 6px -1px rgba(0,0,0,0.1)'; this.style.borderColor='#cbd5e1';" onmouseout="this.style.transform='none'; this.style.boxShadow='none'; this.style.borderColor='transparent';">
                                            <!-- Avatar -->
                                            <div style="width: 48px; height: 48px; border-radius: 50%; background: #3b82f6; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.2rem; font-weight: bold; flex-shrink: 0; box-shadow: inset 0 -2px 0 rgba(0,0,0,0.15);">
                                                {{ strtoupper(substr($emp->first_name, 0, 1) . substr($emp->last_name, 0, 1)) }}
                                            </div>
                                            <!-- Info -->
                                            <div>
                                                <div style="font-weight: 700; color: #1e293b; font-size: 1.05rem; line-height: 1.2;">{{ $emp->full_name }}</div>
                                                <div style="font-size: 0.85rem; color: #64748b; margin-top: 4px;">
                                                    <span style="font-weight: 600; color: #94a3b8;">ID:</span> {{ $emp->nfp_id }}
                                                </div>
                                                @if($emp->branch)
                                                <div style="font-size: 0.8rem; color: #64748b; margin-top: 4px; display: flex; align-items: center; gap: 4px;">
            ...
            @empty
                <div style="text-align: center; padding: 40px; color: #64748b; font-size: 1.1rem;">
